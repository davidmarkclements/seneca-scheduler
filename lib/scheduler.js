  ...
  seneca.add({ role: name, cmd: 'register' }, function(args, done) {
    ...
    if ('every' in args) {
      ...
      if (keys.length === 1 && keys[0] === 'cron') {
        ...
        args.every = keys.filter(startsWithNumber).reduce(function(o, k) {
    ...
    job = schedule.scheduleJob('for' in args ? date.toDate() : args.every, args.task);
    ...
    // For debug purpose only: console.log('Job = ' + JSON.stringify(job));
    jobs[job.id] = job;
    done(null, { job: job });
  });
